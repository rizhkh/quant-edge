...
from models.rf_model      import _train_rf, _rf_quantile_predict


def run_rf_backtest(df: pd.DataFrame, config: dict, params: dict = None) -> dict:
    """Walk-forward backtest for Random Forest. Same metric keys as other backtests."""
    ...
    X_full = build_rf_features(df, use_spy=use_spy)
    ...
    for i in range(test_start_idx, test_end_idx - forecast_days, step_days):
        ...
        try:
            ...
            X_current     = X_full.iloc[[i - 1]].values
            ...
            X_tr = combined.drop(columns=["_y"]).values
            y_tr = combined["_y"].values

            rf  = _train_rf(X_tr, y_tr, params)
            ...
            mae  = abs(rf_med - actual_final)
            mape = abs(rf_med - actual_final) / (actual_final + 1e-10) * 100

            dir_correct = int((rf_med > current_close) == (actual_final > current_close))
            hits        = sum(1 for p in actual_prices if rf_low <= p <= rf_high)
            cone_hit    = hits / len(actual_prices)

            day_errors = {}
            for day in [1, 5, 10, 20, 30]:
                if day <= len(actual_prices):
                    day_errors[f"day_{day}_mae"] = abs(rf_med - actual_prices[day - 1])
            ...
            print(f"  Warning: RF backtest failed at {test_date}: {e}")
            continue
